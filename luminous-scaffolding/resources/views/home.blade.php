...
<div class="row">
  <div class="col-sm-6">
    ...
    <div class="list-group m-b-md">
      ...
      <a href="{{ route('page', $p->parameters('path')) }}" class="list-group-item">
  ...
  <div class="col-sm-6">
    <div class="m-b clearfix">
      ...
      <p class="pull-right" style="margin:0;padding:0.0625rem 0"><a href="{{ route('post_archive') }}"><span class="fa fa-clock-o"></span> Archives</a></p>
    </div>
    <div class="list-group m-b-md">
      @foreach ($wp->posts('post')->orderBy('created_at', 'desc')->take(5)->get() as $p)
      <a href="{{ route('post', $p->parameters('year', 'month', 'day', 'slug')) }}" class="list-group-item">
        <p class="h5 list-group-item-heading">{{ $p->title }}</p>
        <p class="list-group-item-text">{{ $p->excerpt }}</p>
        <p class="list-group-item-text"><small class="text-muted">{{ $p->date('F j, Y') }}</small></p>
      </a>
